...
def get_current_time_string():
    ...
    return time_str
def broadcast_group_values_to_raster(values_by_group, region_layer, nodata_label=0):
    region_layer = region_layer.astype(np.int64)
    unique_labels = np.unique(region_layer.ravel())
    unique_labels = unique_labels[unique_labels != nodata_label]

    if len(unique_labels) != len(values_by_group):
        raise ValueError(
            f"So luong khong khop: raster co {len(unique_labels)} vung, "
            f"values_by_group co {len(values_by_group)} phan tu"
        )

    lookup_array = np.zeros(unique_labels.max() + 1)
    lookup_array[unique_labels] = values_by_group

    return lookup_array[region_layer]
